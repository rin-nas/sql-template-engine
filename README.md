...
5. **`?field`**, где `field` — это произвольное название. Эта метка-заменитель используется только внури условных блоков и может стоять там в любой позиции.
...
1. Иначе блок будет сохранён (вложенные блоки независимы). При этом:
   ...
   * Если значение метки-заменителя равно `BIND_KEEP`, то метка будет удалена. Фактически `BIND_KEEP` удобно использовать только для меток-заменителей `?field`.
